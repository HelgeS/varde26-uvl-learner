"""Runner: static pairwise bias (the baseline).

Binary pairwise bias (+ optional static n-ary group bias via --group-bias-max).
"Skip-collapse" handles n-ary clauses the binary bias can't represent; missing
completeness clauses are recovered post-CA by the reconstruct pipeline.

Usage:
    uv run python -m runners.pairwise --generate-example --verify
    uv run python -m runners.pairwise models/aircraft_fm.uvl --verify
    uv run python -m runners.pairwise models/ --out-dir results/
"""
...
from uvl_learner.acquire import ALGORITHMS, build_algorithm
from uvl_learner.bias.pairwise import build_bias, build_group_bias
