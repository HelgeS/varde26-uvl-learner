...
class SkipCollapseCAEnv(ActiveCAEnv):
    ...
    def __init__(self, skip_collapse: bool = True):
        super().__init__(
            findc=FindCSkipCollapse(time_limit=300)
            if skip_collapse
            else FindC2(time_limit=300),
            qgen=PQGen(time_limit=300),
        )
    ...
    def add_to_cl(self, C):
        if self._skip_collapse and C is None:
            ...
            if scope_info is not None:
                ...
                print(
                    f"  SkipCollapseCAEnv: added blocking nogood for scope {scope}: {nogood}"
                )
...
def run_experiment(
    ...
    try:
        ...
        # )
        skip_env = SkipCollapseCAEnv(skip_collapse=skip_collapse)
        ca = QuAcq(ca_env=skip_env)
        # ca = MQuAcq2(
        #     ca_env=skip_env,
        #     perform_analyzeAndLearn=False, #analyze_and_learn,
        #     cliques_cutoff=cliques_cutoff,
        # )
        ...
        while not done:
            try:
                ...
                bias = skip_env.instance.bias.copy()
                bias.extend(skip_env.instance.cl)

                current_group_size += 1
                print(f"  Increasing group bias to size {current_group_size} and restarting CA...")
                bias.extend(
                    add_group_bias(
                        variables,
                        group_size=current_group_size,
                        constraint_base=bias,
                    )
                )

                # TODO To drop wrongly learned constraints at earlier levels, we can utilize the cache and replay all queries from it.
                # Everything that was removed is known to be wrong, but what was learned maybe not

                # TODO Can I remove group_size-1 after collapse again?
                # TODO Can I guide query generation to a specific scope size?
                # TODO Can we infer the scope of the constraint that is missing and add it dynamically?
                # TODO Instead of restarting, we could force re-processing the last query with the new bias and see if it is now violated, then continue from there without losing all progress.

                problem = ProblemInstance(
                    variables=variables, bias=bias, # init_cl=skip_env.instance.cl.copy()
                )
                continue
        ...
        if len(learned_instance.bias) <= 100:
